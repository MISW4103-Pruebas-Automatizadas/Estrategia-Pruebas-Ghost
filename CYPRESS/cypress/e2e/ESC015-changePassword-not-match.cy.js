describe('Admin Ghost - Change password with unmatched passwords', function() {
it('Change password with unmatched passwords', function() {
    ...
    cy.get('span').each(($span) => {
        if ($span.text().trim() === 'Change password') {
          ...
          cy.wait(1000);
          cy.xpath('//*[@id=":r1s:"]').type('q1w2e3r4t5y6u7i8o9');
        ...
        cy.get('span').each(($span) => {
          if ($span.text().trim() === 'Change password') {
            cy.wrap($span).click();
            cy.wait(1000);
            cy.contains('span', 'Your new passwords do not match', { timeout: 10000 }).should('be.visible');
          }
        });
      });
